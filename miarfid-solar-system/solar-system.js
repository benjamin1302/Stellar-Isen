...
var orbites = new THREE.Object3D();
...
function loadScene(){
	...
	orbites.add(new THREE.Line(mercurio_orbit,mercuriomat));
	orbites.add(new THREE.Line(venus_orbit,   venusmat));
	orbites.add(new THREE.Line(tierra_orbit,  tierramat));
	orbites.add(new THREE.Line(marte_orbit,   martemat));
	orbites.add(new THREE.Line(jupiter_orbit, jupitermat));
	orbites.add(new THREE.Line(saturno_orbit, saturnomat));
	orbites.add(new THREE.Line(urano_orbit,   uranomat));
	orbites.add(new THREE.Line(neptuno_orbit, neptunomat));
...
function update(){
	...
	switch (effectController.focus){
		...
		case "Mercury" :
		if(effectController.follow) camera.position.set(mercurio.position.x + 4*mercurio_tam,mercurio.position.y+mercurio_tam,mercurio.position.z+4*mercurio_tam);
		camera.lookAt(mercurio.position);
		break;


		case "Venus" :
		if(effectController.follow) camera.position.set(venus.position.x + 4*venus_tam,venus.position.y+venus_tam,mercurio.position.z+4*venus_tam);
		camera.lookAt(venus.position);
		break;


		case "Earth" :
		if(effectController.follow) camera.position.set(tierra.position.x + 4*tierra_tam,tierra.position.y+tierra_tam,tierra.position.z+4*tierra_tam);
		...
		break;
		case "Mars" :
		if(effectController.follow) camera.position.set(marte.position.x + 4*marte_tam,marte.position.y+marte_tam,marte.position.z+4*marte_tam);
		...
		case "Saturn" :
		if(effectController.follow) camera.position.set(saturno.position.x + 4*saturno_tam,saturno.position.y+saturno_tam,saturno.position.z+4*saturno_tam);
		camera.lookAt(saturno.position);
		break;

		case "Uranus" :
		if(effectController.follow) camera.position.set(urano.position.x + 20*urano_tam,urano.position.y+urano_tam,urano.position.z+20*urano_tam);
		camera.lookAt(urano.position);
		break;

		case "Neptune" :
		if(effectController.follow) camera.position.set(neptuno.position.x + 4*neptuno_tam,neptuno.position.y,neptuno.position.z+4*neptuno_tam);
	...
	mercurio.rotation.y+=(2*Math.PI/mercurio_rot*(today-antes))* timeScale;
	venus.rotation.y   +=(2*Math.PI/venus_rot*(today-antes)	)* timeScale;
	tierra.rotation.y  +=(2*Math.PI/tierra_rot*(today-antes))* timeScale;
	luna.rotation.y    +=(2*Math.PI/luna_rot*(today-antes)	)* timeScale;
	marte.rotation.y   +=(2*Math.PI/marte_rot*(today-antes)	)* timeScale;
	jupiter.rotation.y +=(2*Math.PI/jupiter_rot*(today-antes))* timeScale;
	saturno.rotation.y +=(2*Math.PI/saturno_rot*(today-antes))* timeScale;
	urano.rotation.y   +=(2*Math.PI/urano_rot*(today-antes)	)* timeScale;
	neptuno.rotation.y +=(2*Math.PI/neptuno_rot*(today-antes))* timeScale;



	// Translation movements
	mercurio_theta +=Math.PI*2/mercurio_per /365*timeScale*(today-antes);
	venus_theta    +=Math.PI*2/   venus_per /365*timeScale*(today-antes);
	tierra_theta   +=Math.PI*2/  tierra_per /365*timeScale*(today-antes);
	luna_theta     +=  Math.PI*2/    luna_per /365*timeScale*(today-antes);
	marte_theta    +=Math.PI*2/   marte_per /365*timeScale*(today-antes);
	jupiter_theta  +=Math.PI*2/ jupiter_per /365*timeScale*(today-antes);
	saturno_theta  +=Math.PI*2/ saturno_per /365*timeScale*(today-antes);
	urano_theta  +=Math.PI*2/   urano_per /365*timeScale*(today-antes);
	neptuno_theta    +=Math.PI*2/ neptuno_per /365*timeScale*(today-antes);
