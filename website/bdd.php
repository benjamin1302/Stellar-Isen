...
function getAstreId ($name){
    $id = array();
    try {
        $bdd = connexionBDD();
        // set the PDO error mode to exception

        $sql =  "SELECT id FROM `astre` WHERE nom='$name'";
        foreach  ($bdd->query($sql) as $row) {
            $id[0] = $row['id'];
        }
...
function getSatId ($name){
    $id = array();
    try {
        $bdd = connexionBDD();
        // set the PDO error mode to exception

        $sql =  "SELECT id FROM `satellite` WHERE nom='$name'";
        foreach  ($bdd->query($sql) as $row) {
            $id[0] = $row['id'];
        }
...
function insertSatTags($id, $tag){
    try {
        $bdd = connexionBDD();

        $sql =  "INSERT INTO `tag`( `tag`,`satId`) VALUES ('$tag',$id)";
        
        $res = $bdd->query($sql);
        if($res)
        {
            echo "<script>console.log('Requete effectué')</script>";
        }
...
function insertPlanetTags($id, $tag){
    try {
        $bdd = connexionBDD();

        $sql =  "INSERT INTO `tag`( `tag`,`astreId`) VALUES ('$tag',$id)";
